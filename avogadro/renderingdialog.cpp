  ...
  , m_solidPipeline(pipeline)
{
  ...
  // 1.0 is the width the outline has always had; the range above it thickens
  // the line rather than darkening it further.
  m_ui->edStrengthDoubleSpinBox->setMinimum(0.0);
  m_ui->edStrengthDoubleSpinBox->setValue(pipeline.getEdStrength());
  m_ui->edStrengthDoubleSpinBox->setMaximum(5.0);
  m_ui->edStrengthDoubleSpinBox->setDecimals(1);
  m_ui->edStrengthDoubleSpinBox->setSingleStep(0.5);
  m_ui->edStrengthDoubleSpinBox->setEnabled(pipeline.getEdEnabled());
  ...
  connect(m_ui->edEnableCheckBox, SIGNAL(stateChanged(int)),
          SLOT(edEnableCheckBoxChanged(int)));
...
float RenderingDialog::edStrength()
{
  return static_cast<float>(m_ui->edStrengthDoubleSpinBox->value());
}

void RenderingDialog::edEnableCheckBoxChanged(int state)
{
  m_ui->edStrengthDoubleSpinBox->setEnabled(state != Qt::Unchecked);
}
...
void RenderingDialog::saveButtonClicked()
{
  ...
  m_solidPipeline.setEdStrength(edStrength());
  this->accept();
}
